...
function parseArgs(argv) {
    ...
    if (a === "--repo-root") out.repoRoot = next();
      ...
      process.stdout.write(
        [
          ...
          "  --strategy auto|date|slug|numbered  Filename strategy (default: auto)",
  ...
  if (!["auto", "date", "slug", "numbered"].includes(out.strategy))
...
function detectStrategy(adrDir) {
  ...
  // Check for numbered (NNN-slug.md) — must come before date check
  for (const name of md) {
    if (/^\d{3,}-[a-z]/.test(name)) return "numbered";
  }
  for (const name of md) {
...
function main() {
  ...
  let filename;
  let displayTitle = title; // title for the heading (may include ADR-NNN prefix)
  if (strategy === "numbered") {
    const num = nextNumber(adrDir);
    const padded = String(num).padStart(3, "0");
    filename = `${padded}-${slug}.md`;
    displayTitle = `ADR-${padded}: ${title}`;
