...
test.describe("Multi-type cards and source provenance", () => {
  ...
  test.describe("Card type rendering", () => {
  ...
  test.describe("Quiz card interactions", () => {
    ...
    test("multiple-choice quiz: correct answer highlights green", async ({ page }) => {
      const card = page
        .locator(`${cardOfType("quiz")}[data-quiz-variant="multiple_choice"]`)
        .first();
      await expect(card).toBeVisible();
    ...
    test("multiple-choice quiz: wrong answer marks both incorrect and correct", async ({
      page,
    }) => {
      const card = page
        .locator(`${cardOfType("quiz")}[data-quiz-variant="multiple_choice"]`)
