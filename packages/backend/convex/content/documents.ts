...
import { enforceDocumentLimit, resolveTier } from "../access/entitlements";
import type { Tier } from "../access/entitlements";
...
export const updateLearningGoal = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    try {
      ...
      await ctx.scheduler.runAfter(0, internal.content.documentActions.embedLearningGoal, {
        documentId: args.id,
        learningGoal: trimmed,
      });
      return null;
...
export const cascadeDeleteChunksAndSummaries = internalMutation({
  ...
  handler: async (ctx, args) => {
    ...
    if (postDrafts.length > 0 && args.userId) {
      const userId = args.userId;
      const postDraftIds = new Set(postDrafts.map((d) => d._id as string));
      const feedbackRows = await ctx.db
        .query("reactionFeedback")
        .withIndex("by_userId", (q) => q.eq("userId", userId))
        .collect();
      for (const fb of feedbackRows) {
        if (postDraftIds.has(fb.postDraftId as string)) {
    ...
    return {
      ...
      deletedPostDrafts: postDrafts.length,
      deletedReactionFeedback,
    };
  },
});
...
export const retry = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    try {
      ...
      await ctx.scheduler.runAfter(0, internal.indexing.resume.resumeProcessing, {
        documentId: args.id,
      });
    } catch (error) {
      evt.setError(error);
