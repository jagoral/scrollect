...
export const generateHighlightDraftsForDocument = internalAction({
  ...
  handler: async (ctx, { documentId, userId, batchNumber, retryCount }) => {
    ...
    try {
      ...
      const highlights = await ctx.runQuery(internal.highlights.listUnprocessedByDocument, {
        documentId,
        limit: BATCH_SIZE,
      });
      ...
      if (sections.length === 0) {
        ...
        await ctx.runMutation(internal.highlights.markDraftGenerated, {
      ...
      const failedHighlights = await ctx.runQuery(internal.highlights.listUnprocessedByDocument, {
        documentId,
        limit: BATCH_SIZE,
      });
