...
import {
  ...
  insertEarlyAdopterGrantIfMissing,
} from "../access/entitlementGrants";
 ...
 *   ctx.runAction(internal.ops.migrateEarlyAdopters.runGrantEarlyAdopters, {})
 */
      ...
      while (true) {
        ...
        if (eligibleIds.length > 0) {
          const result = await ctx.runMutation(
            internal.ops.migrateEarlyAdopters.grantEarlyAdoptersBatch,
            { userIds: eligibleIds },
          );
