...
export const seedE2EData = action({
  ...
  handler: async (ctx): Promise<{ alreadySeeded: boolean; postCount: number }> => {
    ...
    // Idempotency check
    const docCount = await ctx.runQuery(internal.ops.testing.countUserDocuments, {
      userId: user._id,
    });
    if (docCount > 0) {
    ...
    // Insert seeded data
    const result = await ctx.runMutation(internal.ops.testing.insertSeededData, {
      userId: user._id,
      storageId,
    });
...
export const seedE2EDataByEmail = internalAction({
  ...
  handler: async (ctx, args): Promise<{ alreadySeeded: boolean; postCount: number }> => {
    ...
    const user = await ctx.runQuery(internal.ops.testing.findUserByEmail, { email: args.email });
    if (!user) {
