...
import { CHUNK_STORE_BATCH_SIZE, fetchMarkdownBlob } from "./storage";
  ...
  handler: async (ctx, { documentId, markdownStorageId, inferTitle }) => {
    ...
    try {
      ...
      for (let i = 0; i < chunks.length; i += CHUNK_STORE_BATCH_SIZE) {
        ...
        const ids = await ctx.runMutation(internal.content.chunks.createBatch, {
          documentId,
          chunks: batch,
        });
...
async function inferDocumentTitleFromFirstChunk(opts: {
  ...
  try {
    ...
    if (cleanedTitle && cleanedTitle !== opts.currentTitle) {
      await opts.ctx.runMutation(internal.content.documents.updateMetadata, {
        id: opts.documentId,
        title: cleanedTitle,
      });
