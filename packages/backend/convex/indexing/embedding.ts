...
import { EMBED_BATCH_SIZE, MAX_EMBED_RETRIES } from "./storage";
...
export async function fanOutEmbedding(
  ...
  for (let i = 0; i < chunkIds.length; i += EMBED_BATCH_SIZE) {
    ...
    await ctx.scheduler.runAfter(0, internal.indexing.embedding.embedBatch, {
      jobId,
      documentId,
  ...
  handler: async (ctx, { jobId, documentId, chunkIds, retryCount }) => {
    ...
    try {
      const chunks = await Promise.all(
        chunkIds.map((id) => ctx.runQuery(internal.content.chunks.getInternal, { id })),
      );
      ...
      if (result.embeddedChunks.length > 0) {
        await ctx.runMutation(internal.content.chunks.markEmbeddedBatch, {
      ...
      const job = await ctx.runMutation(internal.indexing.processingJobs.markBatchComplete, {
        id: jobId,
        failed: false,
      });
      ...
      if (retryCount < MAX_EMBED_RETRIES) {
        ...
        await ctx.scheduler.runAfter(delayMs, internal.indexing.embedding.embedBatch, {
          jobId,
          documentId,
          chunkIds,
