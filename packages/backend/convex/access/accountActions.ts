...
import { executeDocumentDeletionCascade, markDocumentDeletionFailed } from "../content/deletion";
...
export const deleteAccount = action({
  ...
  handler: async (ctx) => {
    ...
    try {
      ...
      const documentIds = await ctx.runQuery(internal.access.account.getUserDocumentIds, {
        userId: user._id,
      });
      ...
      const remainingResult = await ctx.runMutation(
        internal.access.account.deleteRemainingUserData,
        {
          userId: user._id,
        },
      );
