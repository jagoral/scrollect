...
export const discover = internalAction({
  ...
  handler: async (ctx, { documentId }) => {
    ...
    try {
      ...
      const newDocSections = await ctx.runQuery(internal.content.sectionSummaries.listByDocument, {
        documentId,
      });
      ...
      const readyDocs = await ctx.runQuery(internal.content.documents.listReadyByUser, {
        userId,
      });
      ...
      const otherSections =
        otherDocIds.length > 0
          ? await ctx.runQuery(internal.content.sectionSummaries.listByDocuments, {
      ...
      const chunkResults = await Promise.all(
        Array.from(relevantDocIds).map((docId) =>
          ctx.runQuery(internal.content.chunks.listByDocumentInternal, {
      ...
      const existingPairRecords = await ctx.runQuery(
        internal.drafting.connectionPairs.listPairKeysByUserId,
        { userId },
      );
      ...
      if (result.pairs.length > 0) {
        await ctx.runMutation(internal.drafting.connectionPairs.createBatch, {
