...
export const generateDraftsForSectionBatch = internalAction({
  args: {
    ...
    postTypes: v.array(draftPostType),
  ...
  handler: async (
    ...
    try {
      ...
      const section = await ctx.runQuery(internal.content.sectionSummaries.getInternal, {
        id: sectionSummaryId,
      });
      if (!section) throw new Error(`Section ${sectionSummaryId} not found`);

      const sectionChunks = await ctx.runQuery(internal.content.chunks.listByDocumentRange, {
        documentId,
      ...
      const result = await generateDraftsForSection({
        input: {
          ...
          sectionQualitySignal,
          allChunks: chunkData,
          postTypes: postTypes as DraftPostType[],
          generationBatch,
          contextDepth: "full",
          existingHashes,
